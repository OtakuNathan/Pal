...
Accepted plan artifacts are consumed as DAGs. `metadata.workflow_next` declares the default executor profile for that DAG, while module metadata or topology nodes may declare `executor_profile` for the rare mixed-DAG node that needs a different profile. The manager records `plan_execution.dag_execution.default_executor_profile` and `node_executors`, then schedules ready nodes mechanically from that table.
