# Bunshin State Machine Inventory and Matrix Baseline
...
- runner lifecycle in `BunshinRunState.status`
- coarse database projection in `bunshin_work_orders.status`
...
| run | One live or terminal bunshin runner invocation. Multiple runs may belong to one work order. |
...
`bunshin_work_orders.status` is currently a lossy projection. For example,
...
`metadata.workflow` is initialized by `op_bunshin_dispatch_workflow` and carries
...
`op_bunshin_accept_reviewed_plan` currently performs two separate domain actions:
...
The DAG transition logic in `src/pal/bunshin/dag_advancer.py` is the cleanest
...
- `intro_bunshin_list`, `intro_bunshin_read`: live/terminal runs
- `intro_bunshin_task_search`, `intro_bunshin_task_read`: task facts
- `intro_bunshin_work_order_search`, `intro_bunshin_work_order_read`: durable work-order facts
- `intro_bunshin_work_order_draft_search`, `intro_bunshin_work_order_draft_read`
- `intro_bunshin_profile_list`, `intro_bunshin_profile_read`
- `intro_bunshin_plan_search`, `intro_bunshin_plan_read`

`intro_bunshin_work_order_read` is the correct source for overall work-order
progress. `intro_bunshin_read` answers what a particular live runner is doing.

### Body/Public Operations

Lifecycle and configuration:

- `op_bunshin_attach`, `op_bunshin_detach`
- `op_bunshin_configure`, `op_bunshin_flush_runtime_config`
- `op_bunshin_kill`, `op_bunshin_destroy_work_order_run`
- `op_bunshin_archive_work_order`, `op_bunshin_remove_work_order`

Tasking and dispatch:

- `op_bunshin_task_create`
- `op_bunshin_draft_work_order`, `op_bunshin_promote_work_order_draft`
- `op_bunshin_dispatch_workflow`: normal end-to-end entrypoint
- `op_bunshin_finalize`

Review and execution control:

- `op_bunshin_review_gate_submit`
- `op_bunshin_accept_reviewed_plan`: currently accept **and** dispatch
- `op_bunshin_request_reviewed_plan_revision`
- `op_bunshin_tick_parent_dag`
- `op_bunshin_submit_repair_bill`
- `op_bunshin_recover_work_order`
- `op_bunshin_resume_work_order`
- `op_bunshin_pause_work_order`
...
- `llm_usage_receipt`: idempotently records Bunshin-normalized usage after the
...
### Bunshin-Scoped Capability Groups
...
| Group | Purpose |
| --- | --- |
| `core_bunshin_read` | Bunshin-local facts and artifacts |
| `workspace_read` | tree/search/file/git read access |
| `web_research` | web search/read where permitted |
| `bunshin_plan_reader` | read/find/get/validate plan nodes |
| `bunshin_plan_builder_sketch` | sketch topology, module contracts, module AC, gates, constraints, decisions, submit sketch |
| `bunshin_plan_builder_module_detail` | bound-module interfaces, milestones, AC, validate and submit detail |
| `bunshin_plan_builder_revision` | read and locally revise an existing plan; excludes begin/finalize |
| `bunshin_review_gate` | submit plan/checkpoint/gate-contract review results |
| `code_work` | file mutation, git, shell, checkpoint commit |
| `bunshin_checklist` | milestone checklist read/update |
| `bunshin_repair_bill_builder` | structured local replay or architecture-defect escalation |
...
| --- | --- |
| `op_bunshin_dispatch_workflow` | Create work order and optionally start its configured producer. |
| `op_bunshin_submit_plan_candidate` | First-class entry for an external or programmatically generated valid plan candidate. |
| `op_bunshin_request_reviewed_plan_revision` | Apply `REQUEST_EDIT`/`START_REVISION`; no execution side effects. |
| `op_bunshin_accept_reviewed_plan` | Apply governance `ACCEPT` only. |
| `op_bunshin_start_plan_execution` | Start coder DAG from an accepted plan ref. |
| `op_bunshin_pause_work_order` | Apply execution `PAUSE`. |
| `op_bunshin_resume_work_order` | Resolve aggregate kind, then dispatch one declared `RESUME` action. |
| `op_bunshin_submit_repair_bill` | Apply execution/node repair transition. |
| `op_bunshin_archive_work_order` | Administrative projection/lifecycle, not a substitute for cancellation. |
