...
Minion compact is separate from Pal compact. It uses `pal.compaction.minion.v1` and renders as `<compact_context kind="minion" authority="reference_only">`. It is mechanical and does not use an LLM compaction prompt: it keeps only prior committed user/task inputs as already-handled history, while the current active milestone turn and current tool protocol remain live. Work order, milestone, checkpoint, checklist/todo, repair, cursor, and DAG facts stay in the minion repository/ledger as source of truth rather than being repeated in compact. It does not generate `memory_candidates`; reusable lessons or case memory are proposed at work-order terminal/finalization time.
