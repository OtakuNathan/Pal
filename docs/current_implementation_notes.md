...
- Task Flow, Behavior Guidance Guide, Resident Affordances, and active advisor hints come from behavior providers.
...
`<advisor_hints>` is behavior-owned runtime context. Behavior advice uses its own active hint lifecycle and is no longer projected through memory L2.
