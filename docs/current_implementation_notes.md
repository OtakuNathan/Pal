...
Recalled bodies reach the model only through recall tool results and their output snapshots.
L2 entries are not injected again on subsequent rounds or turns. This applies to
Pal and Bunshin; the continuity summary remains on the existing compact/L1 path.
The behavior advisor has been removed; manuals remain on-demand skills.
...
`channel_provider_rescan` computes a physical manifest diff. Added providers are validated, receive internal endpoint hubs immediately, and auto-attach eligible rows; existing providers are not source-fingerprinted or reloaded; removed/disabled providers withdraw LLM capabilities first, stop their transports, unload code, and delete their hubs. `channel_reload_provider` explicitly performs detach/unload/load/attach, while `channel_restart_endpoint` refreshes one transport without reloading modules. Stable endpoint hubs retain and drain output across lifecycle gaps; physical removal reroutes backlog to the recovery socket, and expected gaps do not enter Safe Mode.
