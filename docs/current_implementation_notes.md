...
The internal allowed pool can be broad, but the LLM-facing tool surface stays small. Normal minion runs expose scoped discovery/read/call plus direct work tools when allowed: `op_exec_shell`, `op_web_search`, `op_web_read`, and `op_memory_recall`. Discovery and read are backed by a scoped runtime view that returns only allowed and non-denied capabilities.
