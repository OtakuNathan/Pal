...
`bunshin` is a detachable first-party plugin/module. Its builtin plugin manifest lives under `runtime_root/plugins/_builtin/bunshin`, while plugin-owned runtime configuration lives under `runtime_root/plugins/bunshin`.
...
- a bunshin event source
- bunshin capability providers
- registered control action handlers

Detach removes all three surfaces. Bunshin business state remains owned by the bunshin subsystem.

## Approval Flow

Bunshin runner emits `approval_requested`.

`BunshinEventSource` converts it into `EventKind.APPROVAL_REQUEST`.

`BunshinControlEventHandler` asks `pal.control.interactions` to compile a typed `interactive_open` delivery and attaches it to a control action. PalCore sends that delivery through the current route. Telegram renders the inline keyboard. Channel-specific fallback belongs in channel endpoints, not in bunshin business logic.

Button clicks use `control.action.dispatch` and produce `bunshin_approval_decision`. PalCore routes that action through `ControlActionHandlerRegistry`. The bunshin module handler calls manager RPC `send_decision` and records the ledger entry.

`op_bunshin_decision_send` is intentionally not exposed to the LLM capability surface.
...
Terminal notifications should name the status, profile, work order, and up to a few artifact paths. Large plans, review reports, and writeups belong in bunshin artifact files, not in chat text.

Pal should answer "what is it doing?" or "where is it now?" by reading bunshin active-run and work-order facts, not by relying on previously pushed chat text.

## Tasking Store

The bunshin subsystem owns these runtime tables in `pal.sqlite3`:

- `bunshin_tasks`
- `bunshin_work_orders`
- `bunshin_work_order_drafts`
- `bunshin_work_order_milestones`
- `bunshin_worker_checkpoints`
- `bunshin_worker_ledger`
- `bunshin_task_lessons`
- `bunshin_system_lesson_candidates`

It also owns FTS tables for task and work order search:

- `bunshin_tasks_fts`
- `bunshin_work_orders_fts`
- `bunshin_work_order_drafts_fts`

One task can have only one active work order. This is enforced by the bunshin repository and a partial unique index over active work order statuses.

## Work Order Drafts

Work order drafts are bunshin-owned planning artifacts. They capture user brainstorming, module boundaries, proposed milestones, acceptance criteria, and workspace hints before the subsystem creates a formal work order. Draft milestones are review input only; they are not an executable milestone truth source.
...
1. Pal captures user brainstorming with `bunshin_draft_work_order`.
...
Pal must not infer work order progress from chat text, progress text, or terminal summaries. The fact source is the bunshin tasking store.

## Workspace And Artifacts

Bunshin work happens inside a task environment. There are two workspace kinds:
...
- Coder reports and test notes are written under `.bunshin/artifacts/{work_order_id}/` and are not part of milestone source commits.

This keeps the bunshin ledger and workspace state aligned: the bunshin tasking store knows which milestone is complete, and Git can restore the files for that milestone.
...
The default prepared repo layout is `data/bunshin/repos/{parent_work_order_id}/{project_name}` for the final inspectable project root. Module work runs in isolated worktrees under `.bunshin/worktrees/{module_name}`, temporary join baselines live under `.bunshin/integrations/{module_name}`, and reports/artifacts live under `.bunshin/artifacts/{work_order_id}`. Explicit `task_repo_path` or `target_repo_path` still takes precedence. `project_name` is read from metadata/workspace when present, otherwise it falls back to the source repo name and then `task_id`. `module_name` is the human-readable module worktree name and remains compatible with existing `module_id` plan artifacts.
...
Bunshin runners always run inside a task sandbox. Sandbox setup is part of manager spawn preparation and is recorded in `TaskContextPack.metadata.sandbox`.
...
- Network remains open so research-capable bunshins can use web tools and package managers when the task permits.
...
- The sandbox gives each run private `HOME`, `TMPDIR`, cache, and pycache paths under `/tmp/pal/bunshin/sandbox/runs/{run_id}` by default, falling back to `runtime_root/data/bunshin/sandbox/runs/{run_id}` when the temp scratch root is unavailable or below the free-space threshold.
...
- `data/bunshin/workspaces/{run_id}_{profile}/work_order.json`
- `data/bunshin/workspaces/{run_id}_{profile}/metadata.json`
...
After user acceptance, the bunshin subsystem may squash the milestone commits into one final work-order commit and merge or apply it to the configured target branch. This finalization is a separate control operation from milestone checkpointing.

## Capabilities

The bunshin module exposes:
...
- `bunshin_detach`
- `bunshin_configure`
- `bunshin_dispatch_workflow`
- `bunshin_review_gate_submit`
- `bunshin_draft_work_order`
- `bunshin_promote_work_order_draft`
- `bunshin_kill`
- `bunshin_finalize`
- `bunshin_tick_parent_dag`
- `bunshin_submit_repair_bill`
- `bunshin_pause_work_order`
- `bunshin_recover_work_order`
- `bunshin_destroy_work_order_run`
...
`bunshin_dispatch_workflow` is the normal public delegation entrypoint. Pal passes `task_id`, the prepared goal, optional `requirements_brief`, workspace facts, `architecture_mode`, `interaction_mode`, and optional `preferred_endpoint_id`. The task binds `profile_family`; dispatch does not take profile selectors. The manager creates the work order, runs the family DAG producer or generic single-node DAG producer, then consumes the resulting DAG mechanically instead of exposing low-level runner mechanics.

Accepted or mechanically generated plan artifacts are consumed as DAGs. Module metadata or topology nodes may declare `role_profile` for a node that needs a different executor from the DAG default. The manager records `plan_execution.dag_execution.default_role_profile` and `node_executors`, then schedules ready nodes mechanically from that table.

`bunshin_draft_work_order` and `bunshin_promote_work_order_draft` remain auxiliary paths for user brainstorming and draft reuse. Draft milestones are review input only; they should be promoted or folded into a normal workflow before execution work starts.
...
`bunshin_dispatch_workflow` may accept optional `preferred_endpoint_id`. Pal should set it only when the user explicitly asks for a specific model or LLM endpoint for that bunshin, after resolving the request to an enabled endpoint id. If omitted, the runner follows the normal Pal active endpoint setting from the runtime database.

Natural-language bunshin control should resolve facts first. For requests like "what is it doing?", "replace it", "tick this DAG", or "merge the completed work", Pal should inspect active runs and work order snapshots, then call the relevant operation. Pal must not infer current worker, progress, or milestone completion from conversation text.

## Runner Continuity

The manager resolves the requested profile, applies role defaults and capability exposure hooks, then asks the bunshin repository to prepare continuity.
...
The manager repeats this preparation before launching the runner, so reattach/retry uses live bunshin-owned facts.

## Compact And Resume Context

Bunshin compact is a task-state recovery packet, not a Pal conversation summary and not a memory-writing path.

Runtime uses a dedicated bunshin compact schema:

- `pal.compaction.bunshin.v1`（历史 v1 契约；当前 runtime 使用
  `pal.compaction.bunshin.v3`，不得作为新实现或恢复格式）

The bunshin compact path and renderer are separate from Pal compact. Bunshin compact is mechanical and does not use an LLM compaction prompt. It collects committed user/task inputs before the current active milestone turn, scrubs management ids, and renders them as already-handled or superseded history. The current active milestone user message and current tool protocol are hard-kept by the live turn and are not compacted.

The rendered context is:

- `<compact_context kind="bunshin" authority="reference_only">`
- title: `Bunshin Task Continuity Reference`
...
Bunshin compact must not create durable memory candidates. Reusable lessons or case memories are proposed at work-order terminal/finalization time and then routed through Pal approval.

## Profiles

Bunshin profiles are declarative. Builtin profiles are package TOML templates, and runtime profiles are loaded from `runtime_root/plugins/bunshin/profiles/*.toml`.
...
The reviewer must submit the verdict with `op_bunshin_review_gate_submit` and `gate_kind=plan_acceptance`.
...
The manager spawns a reviewer work order for the claimed checkpoint. The review target includes the checkpoint id, work order id, run id, module/milestone ids, acceptance criteria, source contract, commit SHA, checkpoint git context, and gate spec. Checkpoint and repair reviewers should submit through `op_bunshin_review_checkpoint`; the generic `op_bunshin_review_gate_submit` surface is hidden for these targets.
...
The runner is allowed to execute task capabilities, report milestone events, and request approval. It must not manage Pal or the bunshin subsystem.
...
Terminal bunshin events may carry:
...
Lessons are never absorbed silently. If a terminal event carries lessons, Pal opens a `bunshin_lesson_approval` interaction with:
...
Pure `memory_candidates` from a terminal event use the generic `memory_candidate_approval` interaction instead of the bunshin lesson interaction. Accept routes through Pal's `memory_candidate_decision` handler and writes approved candidates through `op_memory_write` when available. This keeps tasking lessons, system lesson absorption, and durable case/fact memory candidates separate.
