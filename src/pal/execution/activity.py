...
class _Capture:
    ...
    def observe(self, alias: str, raw: Any, *, background_execution: bool = False) -> None:
        if alias != self.tool:
            return
        ...
        if background_execution or alias == "run_shell":
...
def capture_activity_output(alias: str, raw: Any, *, background_execution: bool = False) -> None:
    capture = _CAPTURE.get()
    if capture is not None:
        try:
            capture.observe(alias, raw, background_execution=background_execution)
        except Exception:
