...
- tool-result delivery metadata is stored on the L1 `ToolResultIR`, so file
  ...
  transcript. Settled results remain intact until prompt projection or
  compaction removes them; their file grants follow that visibility boundary.
  Pager payloads stay in the logical execution runtime and retain their
  independent bounded replay TTL
