        ...
        fragments = [
            PromptFragment(
                ...
                content=(
                    ...
                    "1. Casual chat\n"
                    "- Answer directly only for casual conversation, greetings, simple reactions, or short replies that need no facts, files, tools, memory, design judgment, or external/runtime state.\n"
                    "- Code, system, design, debugging, configuration, project, or memory-related requests are not casual chat.\n\n"
                    "2. Advisor Gate\n"
                    "- For code understanding, debugging, design, review, refactor, research, configuration, plugin/runtime state, repair, multi-step, risky, unclear, or user-specific work, call `op_behavior_advise` before reading files or taking action.\n"
                    "- \"Look at/read/check the code\" is NOT a primitive read when the goal is understanding, diagnosis, design, review, or change.\n"
                    "- Skip advisor only when an active route, injected skill, or current conversation already gives the route.\n\n"
                    "3. Primitive single-capability task\n"
                    "- Use the direct capability shortcut only for narrow primitive requests: read this exact file, list this known object, get current time/status, or call an explicitly requested capability.\n"
                    "- Resolve the capability with `op_exec_disc_search` before use. Do not use this shortcut for codebase analysis or system work.\n\n"
                    "4. Advice result\n"
