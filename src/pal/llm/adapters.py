...
class LLMProviderRegistry:
    def __init__(self, *, default_adapter: type[LLMProviderAdapter] | None = None) -> None:
        ...
        self._default_adapter: type[LLMProviderAdapter] = default_adapter or OpenAIChatProvider
...
class ZaiGLMProvider(LLMProviderAdapter):
    ...
    @classmethod
    def matches_endpoint(cls, endpoint: LLMEndpointModel) -> bool:
        return _is_glm_identifier(
            provider=str(getattr(endpoint, "provider", "") or ""),
            model_id=str(getattr(endpoint, "model_id", "") or ""),
...
def _supports_glm_thinking(endpoint: LLMEndpointModel) -> bool:
    capabilities = _capabilities(endpoint)
    return bool(
        getattr(endpoint, "supports_reasoning", False)
        or capabilities.get("supports_thinking")
        or _adapter_name(endpoint) in ZaiGLMProvider.adapter_names
        or str(getattr(endpoint, "model_id", "") or "").strip().lower().startswith("glm-")
    )
...
def _is_glm_identifier(*, provider: str, model_id: str, base_url: str) -> bool:
    provider_text = str(provider or "").strip().lower()
    model_text = str(model_id or "").strip().lower()
    base_text = str(base_url or "").strip().lower()
    return (
        provider_text in {"zhipu", "zai"}
        or model_text.startswith("glm-")
        or "bigmodel.cn" in base_text
        or "z.ai" in base_text
        or "zhipu" in base_text
    )
