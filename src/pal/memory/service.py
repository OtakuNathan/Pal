...
class MemoryService(MemoryServicePort):
    ...
    def l1_source_stamp(self) -> str:
        """Owner-issued stamp over the current ordered L1 turns."""
        return source_stamp_for_turns(self.l1_store.turns.turns)

    def mark_compaction_cleanup_pending(self, op_id: str) -> None:
        """Record that a committed install's cleanup is still owed (I07)."""
        receipt = self.compaction_receipts.get(str(op_id or "").strip())
        if receipt is None:
            return
        self.compaction_receipts[receipt.op_id] = replace(
    ...
    def compact(self, request: MemoryCompactRequest) -> MemoryCompactResult:
        ...
        if str(request.source_stamp or "").strip():
            return self._compact_full_source(request, summary_entry, projected_entries)
    ...
    def _compact_full_source(
        self,
        request: MemoryCompactRequest,
        summary_entry: L2Entry,
        projected_entries: list[L2Entry],
    ) -> MemoryCompactResult:
        """Whole-generation install: one seed, one fresh segment, one epoch.

        CAS-verified against the captured source stamp and epoch; the receipt
        keyed by op_id is the single durable install fact (I03/I05/I09).
        """
        op_id = str(request.op_id or "").strip()
        if not op_id:
            raise ValueError("full-source compaction requires an op_id")
        stamp = str(request.source_stamp or "").strip()
        existing_receipt = self.compaction_receipts.get(op_id)
        if existing_receipt is not None:
            if existing_receipt.source_stamp != stamp:
                raise ValueError(
                    "compaction op_id replayed with a different source; refusing"
                )
            return MemoryCompactResult(
                summary=summary_entry.summary,
                projected_entries=projected_entries,
                metadata={
                    ...
                    "compact_summary_count": 1,
                    "retired_count": 0,
                    "compaction_op_id": op_id,
                    "compaction_replayed": True,
                    "context_epoch": existing_receipt.epoch_after,
                },
            )
        live_active = [
            turn.turn_id for turn in self.l1_store.turns.turns
            if turn.state == L1TurnState.ACTIVE
        ]
        if len(live_active) > 1:
            raise ValueError(
                "full-source compaction refuses multi-owner active sources: "
                + ",".join(live_active)
            )
        successor_turn_id = str(request.active_turn_id or "").strip()
        if successor_turn_id:
            if successor_turn_id not in live_active:
                raise StaleCompactionSource(
                    f"successor active turn is no longer live: {successor_turn_id}"
                )
        elif live_active:
            raise StaleCompactionSource(
                "active turn appeared after capture: " + ",".join(live_active)
            )
        if source_stamp_for_turns(self.l1_store.turns.turns) != stamp:
            raise StaleCompactionSource("L1 source changed since capture")
        if int(self.context_epoch) != int(request.expected_epoch):
            raise StaleCompactionSource("context epoch moved since capture")

        removed_turn_ids = tuple(
            turn.turn_id for turn in self.l1_store.turns.turns
        )
        removed_result_refs = tuple(
            dict.fromkeys(
                part.call_id
        ...
        summary_turn = _turn_from_transcript("compact-summary", summary_transcript)
        successor: L1TurnIR | None = None
        if successor_turn_id:
            previous = self.l1_store.turns.get(successor_turn_id)
            previous_messages = (
                previous.messages if previous is not None else ()
            )
            previous_metadata = (
                thaw_json(previous.metadata) if previous is not None else {}
            )
            carried_coverage = dict(
                previous_metadata.get("observation_coverage") or {}
            )
            retained_result_refs = tuple(
                dict.fromkeys(
                    part.call_id
            ...
            successor_metadata: dict[str, Any] = {
                "compact_successor": True,
                "compact_op_id": op_id,
            }
            if carried_coverage:
                # Observation coverage is a schema-defined continuity field:
                # it migrates to the successor so the next prompt keeps its
                # exclusion proofs (PLAN 7.3; F24 whitelisted migration).
                successor_metadata["observation_coverage"] = carried_coverage
            if retained_result_refs:
                # The successor keeps the logical references of the active
                # turn's accepted results: retirement must not revoke their
                # execution-runtime deliveries/leases (F25/A05).
                successor_metadata["compact_retained_result_refs"] = [
                    str(ref) for ref in retained_result_refs
                ]
            successor = L1TurnIR(
        ...
        # Single commit section: every statement below is non-throwing local
        # state assignment (I04/I09 — seed, segment, epoch, receipt together).
        self.l1_store.turns.replace_all(
            [summary_turn] + ([successor] if successor is not None else [])
        )
        self.remove_projected_entries([SUMMARY_ENTRY_ID])
        epoch_before = int(self.context_epoch)
        self.context_epoch = epoch_before + 1
        summary_source_id = str(
            getattr(summary_entry, "entry_id", "") or ""
        ).strip() or hashlib.sha256(
            (summary_entry.summary or "").encode("utf-8")
        ).hexdigest()[:16]
        receipt = CompactionReceipt(
            ...
            successor_revision=(successor.revision if successor is not None else 0),
            removed_turn_ids=removed_turn_ids,
            removed_result_refs=removed_result_refs,
            cleanup_status="ok",
            created_at=utc_now(),
        )
        self.compaction_receipts[op_id] = receipt
        while len(self.compaction_receipts) > 256:
            oldest = next(iter(self.compaction_receipts))
            self.compaction_receipts.pop(oldest, None)
        return MemoryCompactResult(
            summary=summary_entry.summary,
            projected_entries=projected_entries,
            metadata={
        ...
        *,
        ...
        if str(request.source_stamp or "").strip():
            ...
            try:
                ...
                # F21/I07: a committed generation change is never rolled back
                # because cleanup failed; the receipt keeps the pending status.
                self.mark_compaction_cleanup_pending(str(request.op_id))
            return result
        previous_l1_turns = list(self.l1_store.turns.turns)
        previous_l2_items = dict(self.l2_store.items)
        previous_top_of_mind_refs = list(self.l2_store.top_of_mind_refs)
