...
Workspace environment preparation is also template-backed. Prefer creating or updating a runtime template under:

```text
<runtime_root>/plugins/minion/workspace_environment/<preparer_id>.toml
```

Do not edit package builtin templates under `src/pal/minion/workspace_environment_templates/` unless the user is intentionally upstreaming a first-party default. Runtime workspace environment templates override builtin templates with the same `preparer_id`. The core executor is still `src/pal/minion/workspace_environment.py` and uses the `WorkspaceEnvironmentPreparer` shape internally, but new language setup should be declarative first. Do not add language-specific logic to `git_env.py`; git env should only call the general workspace environment hook.
...
## Workspace Environment Template Shape

The template-backed fallback lets a new language work when a runtime LSP template advertises a matching `language_ids` entry. Add a workspace environment template when the minion should prepare runtime env vars, path entries, baseline files, or language-specific setup summaries:
...
- `language_ids`: canonical ids used by planner metadata and workspace setup.
...
1. Keep the generic behavior in `src/pal/minion/workspace_environment.py`.
2. Preserve the `WorkspaceEnvironmentPreparer` contract and idempotency rules.
3. Never overwrite user files.
4. Keep any created repo files small and deterministic.
5. Let `git_env.py` commit created files into the workspace baseline; do not call git from the preparer.
...
4. Write workspace setup to `<runtime_root>/plugins/minion/workspace_environment/<preparer_id>.toml`.
5. If needed, add aliases/extensions or generic loader behavior in `workspace_environment.py`.
6. Validate the LSP template with `pal.lsp.config.load_lsp_server_file` or an isolated unit test.
7. Run the focused workspace environment tests for template-backed setup and any new preparer behavior.
8. Run LSP rescan through `op_lsp_mgmt_rescan` when the running Pal runtime should see the new LSP template.
9. Verify with `op_lsp_status`, `op_lsp_doctor`, and a representative diagnostics/hover request when a sample file exists.
...
def builtin_declared_skills(*, module_id: str = "skill") -> tuple[SkillDescriptor, ...]:
    return (
        ...
        SkillDescriptor(
            ...
            summary="Develop runtime-root LSP server templates and matching minion workspace environment templates.",
            ...
            applicability_star=SkillApplicabilitySTAR(
                ...
                task="Create a runtime-root LSP server template and any required workspace environment template.",
                action="Use the template shapes, runtime config paths, preparer contract, and verification checklist.",
            ...
            metadata={
                "internal": True,
                "runtime_root_layout": "plugins/lsp/servers/<server_id>.toml; plugins/minion/workspace_environment/<preparer_id>.toml",
