...
HARNESS_PROTOCOL_VERSION = "minion_harness.v1"
PAL_HARNESS_ID = "pal"
...
class MinionHarnessSpec:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "MinionHarnessSpec":
...
@dataclass(frozen=True)
class MinionHarnessRegistryGeneration:
    generation_hash: str
    specs: tuple[MinionHarnessSpec, ...]

    def select(self, role: str) -> MinionHarnessSpec:
        candidates = [spec for spec in self.specs if spec.supports(role)]
        if not candidates:
            raise LookupError(f"no Minion harness supports role {role}")
        return sorted(
            candidates,
...
def pal_harness_spec() -> MinionHarnessSpec:
...
class MinionHarnessRegistry:
    ...
    def __init__(self, *, include_pal: bool = False) -> None:
        ...
        self._listeners: list[
            Callable[[MinionHarnessRegistryGeneration], None]
        ] = []
        self._specs: dict[str, MinionHarnessSpec] = {}
        if include_pal:
            pal = pal_harness_spec()
            self._specs[pal.harness_id] = pal
        self._generation = self._compile()

    def snapshot(self) -> MinionHarnessRegistryGeneration:
        with self._lock:
            return self._generation

    def register(self, spec: MinionHarnessSpec) -> None:
        with self._lock:
    ...
    def replace_external(
        ...
        raw_specs = [
            MinionHarnessSpec.from_mapping(dict(item))
    ...
    def subscribe(
        self,
        listener: Callable[[MinionHarnessRegistryGeneration], None],
    ) -> Callable[[], None]:
        with self._lock:
