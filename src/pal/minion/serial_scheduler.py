...
class SerialMilestoneScheduler:
    ...
    async def _send_serial_module_turn(self, state: Any, event: dict[str, Any], inflight_key: str) -> None:
        ...
        try:
            ...
            auto_tick_parent_work_order_id = ""
            ...
            if str(completion.get("status") or "") == "completed":
                ...
                if str(parent_completion.get("status") or "") in {"awaiting_continue", "running_module", "completed"}:
                    ...
                    if str(parent_completion.get("status") or "") in {"awaiting_continue", "running_module"} and bool(parent_completion.get("has_next_module")):
                        auto_tick_parent_work_order_id = event_work_order_id
            ...
            if auto_tick_parent_work_order_id:
                await manager.auto_tick_parent_dag(auto_tick_parent_work_order_id, reason="module_completed")
            elif schedule_global_ready:
                with contextlib.suppress(Exception):
                    await manager.tick_ready_plan_dags(reason="module_completed")
            try:
