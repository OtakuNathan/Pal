...
from pal.minion.catalog_store import load_json_objects
from pal.minion.config import (
    MINION_DB_FILENAME,
    ensure_minion_runtime_settings_schema,
)
from pal.minion.ipc import MinionManagerClient, minion_runtime_dir
...
class MinionRuntimeCutoverResult:
    status: str
...
def cutover_minion_runtime_v25(runtime_root: Path) -> MinionRuntimeCutoverResult:
    """Archive a legacy Minion runtime and create one canonical v25 root."""

    root = Path(runtime_root)
    source = minion_runtime_dir(root)
    previous_version = _minion_schema_version(source / MINION_DB_FILENAME)
    if previous_version in {0, MINION_V2_SCHEMA_VERSION}:
        return MinionRuntimeCutoverResult(
            status=(
                "already_current"
                if previous_version == MINION_V2_SCHEMA_VERSION
                else "not_required"
            ),
    ...
    data_root = root / "data"
    archive_parent = data_root / "minion-archive"
    ...
    staging_runtime_root = data_root / f".minion-v25-{uuid4().hex}"
    ...
    try:
        ...
        MinionCatalogService(staging_runtime_root).bootstrap()
...
def _build_staging_runtime(
    ...
    *,
    ...
    for path, _payload in families:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(path, target)
    with sqlite3.connect(str(staging / MINION_DB_FILENAME)) as connection:
...
def _minion_schema_version(db_path: Path) -> int:
...
def _require_manager_stopped(runtime_root: Path) -> None:
    ...
    if bool(health.get("ok")):
        raise RuntimeError(
            "Minion runtime cutover requires the Minion sidecar to be detached "
