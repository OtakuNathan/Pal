...
def minion_data_root(runtime_root: Path) -> Path:
    return Path(runtime_root) / "data" / "minion"


def artifact_store_root(runtime_root: Path) -> Path:
    return minion_data_root(runtime_root) / "artifacts" / "sha256"


def runtime_spool_root(runtime_root: Path) -> Path:
    return minion_data_root(runtime_root) / "runtime"
...
def deliverable_root(runtime_root: Path) -> Path:
    return minion_data_root(runtime_root) / "deliverables"
...
def plan_revision_root(
    ...
    *,
    ...
    return minion_data_root(runtime_root) / "plan_revisions" / project_key / workflow_key / revision_key
...
def resolve_project_git_layout(
    ...
    *,
    ...
    repos_root = minion_data_root(runtime_root) / "repos"
    ...
    workflow_branch = str(stored.get("workflow_branch") or f"minion/{workflow_key}/main")
