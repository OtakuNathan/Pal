...
from pal.bunshin.catalog_store import (
    atomic_write_json,
    family_override_path,
    family_override_root,
    load_json_objects,
    bunshin_catalog_root,
    profile_override_path,
    profile_override_root,
    remove_override,
)
from pal.bunshin.families import BunshinFamilyManifest, BunshinFamilyRegistry
...
class BunshinCatalogService:
    ...
    def bootstrap(self) -> dict[str, Any]:
        with self._lock:
            root = bunshin_catalog_root(self.runtime_root)
    ...
    def set_profile_override(
        ...
        *,
        ...
        with self._lock:
            ...
            registry = BunshinProfileRegistry(runtime_root=self.runtime_root)
            ...
            metadata["managed_by"] = "bunshin_sidecar"
            if BunshinProfileRegistry(runtime_root=None).get_ref(group, name) is not None:
                metadata["override_of"] = canonical_profile_id(group, name)
            else:
                metadata.pop("override_of", None)
            patched["metadata"] = metadata
            model = BunshinProfile.from_dict(patched)
            ...
            effective = BunshinProfileRegistry(runtime_root=self.runtime_root).get_ref(group, name)
            return {
                "status": "updated",
    ...
    def set_family_override(
        ...
        *,
        ...
        with self._lock:
            ...
            current = BunshinFamilyRegistry(runtime_root=self.runtime_root).get(family_id)
            ...
            metadata["builtin"] = False
            metadata["managed_by"] = "bunshin_sidecar"
            if BunshinFamilyRegistry(runtime_root=None).get(family_id) is not None:
                metadata["override_of"] = family_id
            else:
                metadata.pop("override_of", None)
            patched["metadata"] = metadata
            model = BunshinFamilyManifest.from_dict(patched)
            ...
            effective = BunshinFamilyRegistry(runtime_root=self.runtime_root).get(family_id)
            return {
                "status": "updated",
                "family": family_id,
                "generation": after["generation"],
    ...
    def _snapshot(self, *, include_definitions: bool) -> dict[str, Any]:
        ...
        for profile in BunshinProfileRegistry(runtime_root=self.runtime_root).list_profiles():
        ...
        for family in BunshinFamilyRegistry(runtime_root=self.runtime_root).list_families():
    ...
    def _validate_profile(self, profile: BunshinProfile) -> None:
        family_registry = BunshinFamilyRegistry(runtime_root=self.runtime_root)
        unknown: list[str] = []
    ...
    def _validate_family(self, family: BunshinFamilyManifest) -> None:
        registry = BunshinProfileRegistry(runtime_root=self.runtime_root)
    ...
    def _resolve_profile_identity(self, selector: str) -> tuple[str, str]:
        ...
        current = BunshinProfileRegistry(runtime_root=self.runtime_root).get(normalized)
    ...
    @staticmethod
    def _profile_payload(profile: BunshinProfile | None) -> dict[str, Any]:
        if profile is None:
            return {}
        payload = dict(profile.to_dict())
        for key in ("canonical_profile_id", "bunshin_profile", "approval_policy"):
    ...
    @staticmethod
    def _family_payload(family: BunshinFamilyManifest | None) -> dict[str, Any]:
        if family is None:
            return {}
    ...
    def _append_audit(self, payload: Mapping[str, Any]) -> None:
        path = bunshin_catalog_root(self.runtime_root) / "audit.jsonl"
