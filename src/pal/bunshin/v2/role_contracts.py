...
from pal.bunshin.families import BunshinRoleBinding
...
) -> dict[str, BunshinRoleBinding]:
    bindings = {
        str(role).strip(): (
            binding
            if isinstance(binding, BunshinRoleBinding)
            else BunshinRoleBinding.from_payload(str(role), binding)
        )
    ...
    for role in sorted(raw_bindings):
        ...
        if participant == "profile":
            canonical_profile_id = str(
                ...
                or role_profile.get("bunshin_profile")
                or ""
            ).strip()
