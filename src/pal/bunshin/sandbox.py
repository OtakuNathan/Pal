...
from pal.bunshin.ipc import (
    ...
    bunshin_role_socket_path,
)
from pal.bunshin.workspace_tools import _normalized_reference_paths
from pal.shared import BunshinInvocationPack


PAL_BUNSHIN_SANDBOX_SCRATCH_ROOT_ENV = "PAL_BUNSHIN_SANDBOX_SCRATCH_ROOT"
PAL_BUNSHIN_SANDBOX_MIN_FREE_MB_ENV = "PAL_BUNSHIN_SANDBOX_MIN_FREE_MB"
PAL_BUNSHIN_SANDBOX_MAX_RUN_DIRS_ENV = "PAL_BUNSHIN_SANDBOX_MAX_RUN_DIRS"
PAL_BUNSHIN_RUNTIME_ROOT_ENV = "PAL_BUNSHIN_RUNTIME_ROOT"
DEFAULT_BUNSHIN_SANDBOX_MIN_FREE_MB = 256
...
def with_bunshin_sandbox_metadata(runtime_root: Path, pack: BunshinInvocationPack, *, run_id: str) -> BunshinInvocationPack:
    metadata = dict(pack.metadata or {})
    sandbox_config = dict(metadata.get("sandbox") or {})
    if _falsey(sandbox_config.get("enabled")) or _falsey(os.environ.get("PAL_BUNSHIN_SANDBOX")):
        raise RuntimeError("Bunshin execution requires an OS sandbox and cannot be disabled")
    backend = str(sandbox_config.get("backend") or os.environ.get("PAL_BUNSHIN_SANDBOX_BACKEND") or sandbox_supported_backend()).strip()
    if not backend:
        raise RuntimeError("Bunshin execution requires bubblewrap on Linux")
    if backend != "bwrap":
        raise RuntimeError(f"Bunshin execution requires the bubblewrap adapter; unsupported backend: {backend}")
    workspace_path = _workspace_path_from_pack(pack)
    scratch_dir = bunshin_sandbox_scratch_dir(runtime_root, run_id)
    sandbox_metadata = BunshinSandboxSpec(
        enabled=True,
        backend=backend,
...
def _project_sandbox_references(
    ...
    for reference in _normalized_reference_paths(projected_workspace):
        ...
        target = str(BUNSHIN_SANDBOX_REFERENCE_ROOT / _safe_reference_component(name))
...
def bunshin_sandbox_scratch_dir(runtime_root: Path, run_id: str) -> Path:
...
def build_sandboxed_runner_invocation(
    *,
    ...
    if not bool(sandbox.get("enabled")):
        raise RuntimeError("Bunshin runner invocation is missing its required OS sandbox")
    backend = str(sandbox.get("backend") or "").strip()
    if backend == "bwrap":
        final_env = scrub_bunshin_sandbox_env(
    ...
    if backend == "docker":
        raise RuntimeError("macOS Docker bunshin sandbox requires PAL_BUNSHIN_DOCKER_IMAGE; the Docker launcher is not wired yet")
    raise RuntimeError(f"unsupported bunshin sandbox backend: {backend or 'unknown'}")
...
def _coerce_scratch_dir(runtime_root: Path, run_id: str, scratch_dir: str | Path | None) -> Path:
    ...
    return bunshin_sandbox_scratch_dir(runtime_root, run_id)


def _preferred_sandbox_scratch_root(runtime_root: Path) -> Path:
    override = str(os.environ.get(PAL_BUNSHIN_SANDBOX_SCRATCH_ROOT_ENV) or "").strip()
    if override:
        return Path(override).expanduser()
    return pal_log_root(runtime_root) / "bunshin" / "sandbox" / "runs"


def _runtime_sandbox_scratch_root(runtime_root: Path) -> Path:
    return Path(runtime_root) / "data" / "bunshin" / "sandbox" / "runs"
...
def _sandbox_min_free_bytes() -> int:
    return max(0, _env_int(PAL_BUNSHIN_SANDBOX_MIN_FREE_MB_ENV, DEFAULT_BUNSHIN_SANDBOX_MIN_FREE_MB)) * 1024 * 1024


def _sandbox_max_run_dirs() -> int:
    return max(0, _env_int(PAL_BUNSHIN_SANDBOX_MAX_RUN_DIRS_ENV, DEFAULT_BUNSHIN_SANDBOX_MAX_RUN_DIRS))
...
def scrub_bunshin_sandbox_env(
    env: dict[str, str],
    *,
    runtime_root: Path,
    run_id: str,
    pack: BunshinInvocationPack | None = None,
    ...
    result["PAL_BUNSHIN_SANDBOXED"] = "1"
    result.pop("PAL_BUNSHIN_CONTINUATION_RETRY", None)
    result["PAL_BUNSHIN_WEB_BROKER"] = "1"
    result["PAL_DATABASE_READ_ONLY"] = "1"
    result[PAL_BUNSHIN_RUNTIME_ROOT_ENV] = str(Path(runtime_root).expanduser().resolve())
    if assignment_token:
...
def _apply_workspace_execution_env(env: dict[str, str], pack: BunshinInvocationPack) -> None:
...
def ensure_sandbox_files(
    ...
    *,
    ...
    sandbox_root = Path(runtime_root) / "data" / "bunshin" / "sandbox"
    scratch = _coerce_scratch_dir(runtime_root, run_id, scratch_dir)
...
def _build_bwrap_invocation(
    *,
    ...
    if not bwrap:
        raise RuntimeError("bubblewrap is required for Linux bunshin sandboxing")
    ...
    from pal.execution.worker_extensions import worker_dependency_paths
    for dependency_path in worker_dependency_paths(
        Path(runtime_root), env, database_path=env.get(BUNSHIN_RUNTIME_DB_PATH_ENV) or None
    ):
        _append_bind_path(args, dependency_path, read_only=True)
...
def _append_reference_projection_binds(
    ...
    for reference in references:
        ...
        if not source.exists() or not source.is_dir():
            ...
            continue
        if not target.is_absolute() or target.parent != BUNSHIN_SANDBOX_REFERENCE_ROOT:
...
def _append_runtime_root_binds(
    ...
    *,
    ...
    configured_db_path = str(env.get(BUNSHIN_RUNTIME_DB_PATH_ENV) or "").strip()
    database_path = (
    ...
    endpoint_path = bunshin_role_socket_path(runtime_root)
    if not endpoint_path.exists():
        raise RuntimeError(
            "sandboxed Bunshin requires the assignment-scoped Unix role gateway"
        )
...
def _workspace_path_from_pack(pack: BunshinInvocationPack) -> Path | None:
    workspace = dict(pack.workspace or {})
