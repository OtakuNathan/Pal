...
ROLE_GATEWAY_TOKEN_ENV = "PAL_BUNSHIN_ROLE_ASSIGNMENT_TOKEN"
BUNSHIN_RUNTIME_DB_PATH_ENV = "PAL_BUNSHIN_RUNTIME_DB_PATH"


def bunshin_runtime_dir(runtime_root: Path) -> Path:
    return Path(runtime_root) / "data" / "bunshin"


def bunshin_socket_path(runtime_root: Path) -> Path:
    return _bunshin_endpoint(runtime_root).socket_path


def bunshin_port_path(runtime_root: Path) -> Path:
    return _bunshin_endpoint(runtime_root).port_path


def bunshin_role_socket_path(runtime_root: Path) -> Path:
...
DEFAULT_BUNSHIN_MANAGER_REQUEST_TIMEOUT_SECONDS = 300.0


@dataclass
class BunshinManagerClient:
    runtime_root: Path
    request_timeout_seconds: float = DEFAULT_BUNSHIN_MANAGER_REQUEST_TIMEOUT_SECONDS
    _client: SidecarRpcClient = field(init=False, repr=False)

    def __post_init__(self) -> None:
        self._client = SidecarRpcClient(
            endpoint=_bunshin_endpoint(self.runtime_root),
            request_timeout_seconds=self.request_timeout_seconds,
        )

    @property
    def socket_path(self) -> Path:
        return bunshin_socket_path(self.runtime_root)

    async def request(self, method: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        try:
            return await self._client.request(method, params)
        except SidecarRpcError as exc:
            raise BunshinManagerRpcError(str(exc), kind=exc.kind, payload=exc.payload) from exc
...
@dataclass
class BunshinRoleGatewayClient:
    runtime_root: Path
    access_token: str
    request_timeout_seconds: float = DEFAULT_BUNSHIN_MANAGER_REQUEST_TIMEOUT_SECONDS
    _client: SidecarRpcClient = field(init=False, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        self._client = SidecarRpcClient(
            endpoint=_bunshin_role_endpoint(self.runtime_root),
            request_timeout_seconds=self.request_timeout_seconds,
    ...
    @property
    def socket_path(self) -> Path:
        return bunshin_role_socket_path(self.runtime_root)
...
async def open_manager_connection(runtime_root: Path):
    return await open_sidecar_connection(_bunshin_endpoint(runtime_root))


async def start_manager_server(runtime_root: Path, handler):
    return await start_sidecar_server(_bunshin_endpoint(runtime_root), handler)


async def cleanup_manager_endpoint(runtime_root: Path) -> None:
    await cleanup_sidecar_endpoint(_bunshin_endpoint(runtime_root))


async def start_role_gateway_server(runtime_root: Path, handler):
    return await start_sidecar_server(_bunshin_role_endpoint(runtime_root), handler)


async def cleanup_role_gateway_endpoint(runtime_root: Path) -> None:
    await cleanup_sidecar_endpoint(_bunshin_role_endpoint(runtime_root))


def _bunshin_endpoint(runtime_root: Path) -> SidecarEndpoint:
    return SidecarEndpoint(runtime_root=Path(runtime_root), name="bunshin")


def _bunshin_role_endpoint(runtime_root: Path) -> SidecarEndpoint:
