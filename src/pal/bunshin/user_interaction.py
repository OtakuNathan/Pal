...
@dataclass
class BunshinUserInteractionPort:
        ...
        *,
        ...
        await self.emit_event(
            "approval_requested",
            {
                ...
                "title": title or "Bunshin high-risk operation",
                "requested_action": capability_name,
                "risk": risk,
                "impact": impact or "Bunshin requested permission before running a high-risk operation.",
...
def ask_user_question_summary(payload: dict[str, Any]) -> str:
    ...
    if not questions:
        return "bunshin asked a user clarification question"
    first = str(questions[0].get("question") or "bunshin asked a user clarification question").strip()
    if len(questions) == 1:
        return first
