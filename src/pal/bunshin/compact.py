...
COMPACTION_SCHEMA_BUNSHIN_V3 = "pal.compaction.bunshin.v3"

BUNSHIN_COMPACTION_SYSTEM_PROMPT = (
    "You are the Bunshin work-checkpoint compactor.\n"
    "Return one valid JSON object only, using schema pal.compaction.bunshin.v3.\n"
...
_BUNSHIN_TOP_LEVEL_FIELDS = frozenset(
...
class BunshinCompactionPolicy:
    policy_id: str = COMPACTION_SCHEMA_BUNSHIN_V3
    ...
    def build_source(
        ...
        *,
        ...
        lines = [
            '<compact_source kind="bunshin" schema_target="pal.compaction.bunshin.v3">',
            "## Source Semantics",
            "",
    ...
    def validate_checkpoint(
        ...
        if str(payload.get("kind") or "").strip().lower() != "bunshin":
            raise ValueError("kind must be bunshin")
        if "memory_candidates" in payload:
            raise ValueError("bunshin checkpoints must not contain memory_candidates")
        forbidden = _find_forbidden_reasoning_key(payload)
        if forbidden:
            raise ValueError(f"forbidden reasoning field: {forbidden}")
        _reject_extra_fields(payload, _BUNSHIN_TOP_LEVEL_FIELDS, "checkpoint")
        ...
        return _make_bunshin_summary_entry(normalized_payload)


def render_bunshin_compact_context_for_llm(
    *,
    summary: str,
...
def is_bunshin_compaction_payload(payload: object) -> bool:
    return (
        isinstance(payload, dict)
        and str(payload.get("schema") or "").strip()
        == COMPACTION_SCHEMA_BUNSHIN_V3
    )


def _make_bunshin_summary_entry(payload: dict[str, Any]) -> L2Entry:
    ...
    return L2Entry(
        ...
        touched_at=utc_now(),
        rendered=render_bunshin_compact_context_for_llm(
            summary=summary,
            payload=payload,
        ),
...
__all__ = [
    ...
    "is_bunshin_compaction_payload",
    "render_bunshin_compact_context_for_llm",
]
