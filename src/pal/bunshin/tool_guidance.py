        ...
        "op_exec_shell": MappingProxyType(
            {
                ...
                "failure_next_steps": (
                    "For a trapped Git command, use a permitted read-only subcommand or stop. For an ordinary command "
                    "failure, inspect stdout, stderr, and exit status, correct the command or environment, and then "
                    "rerun only when safe."
                ),
            }
        ),
...
def bunshin_tool_guidance(
    ...
    system_patch = BUNSHIN_SYSTEM_TOOL_GUIDANCE_OVERRIDES.get(str(canonical_path or "").strip())
    if system_patch:
...
__all__ = [
    "BUNSHIN_SYSTEM_TOOL_GUIDANCE_OVERRIDES",
    "TOOL_GUIDANCE_FIELDS",
