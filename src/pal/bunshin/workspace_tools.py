...
def _workspace_tool_result(call: ToolCallIR, workspace: dict[str, Any]) -> ToolExecutionResult:
    try:
        if call.name == "op_bunshin_artifact_write":
            artifact = _write_bunshin_artifact(workspace, call.args)
            payload = {"artifact": artifact}
            action = "staged" if artifact.get("staged") else "written"
            text = f"Artifact {action}: {artifact['relative_path']}"
        elif call.name == "op_bunshin_artifact_edit":
            artifact = _edit_bunshin_artifact(workspace, call.args)
            payload = {"artifact": artifact}
...
def _write_bunshin_artifact(workspace: dict[str, Any], args: dict[str, Any]) -> dict[str, Any]:
