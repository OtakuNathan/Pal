...
class BunshinManagerProvider:
    ...
    def __post_init__(self) -> None:
        ...
        self.client = BunshinManagerClient(self.runtime_root)
        self._lifecycle_client = BunshinManagerClient(self.runtime_root, request_timeout_seconds=5.0)
    ...
    async def handle_control_action_async(self, action: ControlAction) -> str:
        if action.action_kind == "bunshin_v2_human_decision":
        ...
        if action.action_kind == "bunshin_approval_decision":
            payload = {
                ...
                "bunshin_id": str(action.args.get("bunshin_id") or ""),
            }
            try:
                await asyncio.to_thread(self._require_manager)
                await asyncio.to_thread(self.client.send_decision_sync, payload)
                return "Bunshin approval decision recorded."
            except Exception as exc:
                return f"Bunshin approval decision failed: {exc}"
        if action.action_kind == "bunshin_question_answer":
            try:
                await asyncio.to_thread(self._require_manager)
                await asyncio.to_thread(self.client.send_clarification_sync, dict(action.args))
                return "Bunshin clarification recorded."
            except Exception as exc:
                return f"Bunshin clarification failed: {exc}"
        return ""
    ...
    def _require_manager(self) -> dict[str, Any]:
        with self._lifecycle_lock:
            if not self._attached:
                raise RuntimeError("bunshin sidecar is detached")
    ...
    def _validate_health(health: dict[str, Any]) -> dict[str, Any]:
        if not bool(health.get("ok")) or str(health.get("health_source") or "") != "bunshin_v2_manager":
            raise RuntimeError("bunshin manager health check failed")
        if str(health.get("lifecycle_protocol") or "") != "plugin_raii.v1":
            raise RuntimeError("bunshin manager lifecycle protocol is incompatible")
        if bool(health.get("shutdown_requested")):
            raise RuntimeError("bunshin manager is shutting down")
        return dict(health)
    ...
    def _manager_is_responding(self) -> bool:
        try:
            ...
            return bool(health.get("ok")) and str(health.get("health_source") or "") == "bunshin_v2_manager"
        except Exception:
            return False
    ...
    def _cleanup_stale_endpoint(self) -> None:
        for path in (self.client.socket_path, bunshin_port_path(self.runtime_root)):
            if path.exists():
    ...
    async def _event_subscription_loop(self) -> None:
        ...
        try:
            ...
            if str(response.get("id") or "") != request_id or not bool(response.get("ok")):
                raise RuntimeError("bunshin event subscription rejected")
            while not self._event_stop.is_set():
...
def register_with_core(
    ...
    *,
    ...
    _ = service
    resolved_root = Path(runtime_root or context.execution_runtime.runtime_root or Path.cwd() / ".pal-bunshin")
    manager = BunshinManagerProvider(
    ...
    source = BunshinEventSource(provider=manager)
    handler = BunshinControlEventHandler(provider=manager)
    event_kinds = (
        EventKind.APPROVAL_REQUEST,
        EventKind.BUNSHIN_TERMINAL,
        EventKind.BUNSHIN_STANDALONE_REVIEW_COMPLETED,
        EventKind.BUNSHIN_CLARIFICATION_REQUEST,
    ...
    handle = ModuleHandle(
        ...
        control_action_handlers={
            "bunshin_approval_decision": manager.handle_control_action_async,
            "bunshin_question_answer": manager.handle_control_action_async,
            "bunshin_v2_human_decision": manager.handle_control_action_async,
        },
        ports={"bunshin": manager, "bunshin_v2": public},
    ...
    context.event_source_registry.attach("bunshin", source)
    for kind in event_kinds:
        context.event_handler_registry.register(kind, handler, module_id="bunshin")
    return handle


def inspect_bunshin(provider: BunshinManagerProvider) -> BunshinSnapshot:
    try:
        health = provider._require_manager()
        return BunshinSnapshot(
            mounted=True,
        ...
        return BunshinSnapshot(mounted=provider._attached, degraded=True)
