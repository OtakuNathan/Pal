...
from pal.bunshin.debug_log import bunshin_debug_log_enabled
...
from pal.bunshin.prompt_adapter import (
    build_bunshin_task_envelope as _bunshin_task_envelope,
    BunshinPromptFragmentProvider,
    bunshin_primary_input as _bunshin_primary_input,
    prompt_scaffold_summary as _prompt_scaffold_summary,
    prompt_view_from_pack as _prompt_view_from_pack,
)
from pal.bunshin.user_interaction import (
    BunshinUserInteractionPort,
...
class BunshinRunner:
    ...
    async def _run_v2_invocation(
        ...
        *,
        ...
        await self._emit(
            "terminal",
            self._terminal_payload("completed", final_text or "Bunshin V2 invocation completed"),
        )
        return 0
    ...
    async def _run_agent_loop(self, bundle: BunshinRuntimeBundle, *, forced_retry_note: str = "") -> str:
        ...
        program = agent_turn_program(
            ...
            build_retry_note=lambda outcome, observations, retry_count: self._build_bunshin_retry_note(
                outcome,
                observations,
                retry_count,
                state=state,
        ...
        while True:
            ...
            try:
                ...
                self._sync_bunshin_state_from_continuation(state, continuation)
            ...
            current = await self._execute_bunshin_agent_effect(
                executor,
                continuation,
                state,
                yielded,
    ...
    async def _read_manager_control(self, timeout: float | None = None) -> dict[str, Any] | None:
        ...
        if message_type in {"cancel_requested", "cancel"}:
            raise _BunshinCooperativeCancel(self._remember_cancel_request(dict(message.get("payload") or message)))
    ...
    async def _raise_if_restart_requested(self) -> None:
        if self._restart_requested:
            raise _BunshinCooperativeRestart(dict(self._restart_requested))
    ...
    def _build_bunshin_agent_runtime(
        self,
        bundle: BunshinRuntimeBundle,
        state: BunshinAgentLoopState,
        continuation: TurnContinuation,
    ) -> AgentTurnRuntime:
        llm_runtime = _BunshinLLMRuntimeAdapter(self, bundle.llm_runtime, state)
        output_port = _BunshinOutputPort(self)
        prompt_fragment_registry = self._build_bunshin_prompt_fragment_registry()
        context = MainContext(
        ...
        return AgentTurnRuntime.build(
            ...
            execute_tool_async=lambda call, **kwargs: self._execute_bunshin_tool_with_observation(
                state,
                continuation,
                call,
                **kwargs,
    ...
    async def _execute_bunshin_agent_effect(
        ...
        *,
        ...
        if isinstance(effect, LLMRequestEffect):
            result = await self._postprocess_bunshin_llm_round(
                state,
                result,
    ...
    def _preflight_bunshin_llm_round(self, state: BunshinAgentLoopState) -> EffectResult | None:
        ...
        if not shell_pending and (self._completion_evidence_present() or self._artifact_completion_evidence_present()):
            ...
            outcome = _bunshin_generation_result(self.blocked_summary)
        return EffectResult(status=RuntimeStatus.OK, payload=outcome)

    async def _postprocess_bunshin_llm_round(
        self,
        state: BunshinAgentLoopState,
        result: EffectResult,
        *,
    ...
    def _sync_bunshin_state_from_continuation(self, state: BunshinAgentLoopState, continuation: TurnContinuation) -> None:
    ...
    def _debug_log_bunshin_llm_request(self, state: BunshinAgentLoopState, request: LLMRequestIR) -> None:
    ...
    def _debug_log_bunshin_llm_outcome(self, state: BunshinAgentLoopState, outcome: Any) -> None:
        context = self._prompt_debug_context(round=state.llm_round_count)
    ...
    async def _execute_bunshin_tool_with_observation(
        self,
        state: BunshinAgentLoopState,
        continuation: TurnContinuation,
        call: ToolCallIR,
        *,
        ...
        try:
            operation = self._execute_allowed_tool(
    ...
    async def _execute_allowed_tool(
        ...
        *,
        ...
        policy_call = self._tool_call_with_bunshin_defaults(admission.call)
    ...
    def _tool_call_with_bunshin_defaults(self, tool_call: ToolCallIR) -> ToolCallIR:
    ...
    def _user_interaction_port(self) -> BunshinUserInteractionPort:
    ...
    def _default_text_artifact_path(self, *, suffix: str = "", partial: bool = False) -> str:
        ...
        return f"invocation_{self._safe_path_part(self.pack.bunshin_profile)}{suffix}.md"
    ...
    def _cancel_terminal_payload(self, cancel: dict[str, Any]) -> dict[str, Any]:
        payload = self._terminal_payload("killed", cancel.get("summary") or cancel.get("reason") or "bunshin cancellation requested")
