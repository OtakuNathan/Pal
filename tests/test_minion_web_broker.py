...
from pal.shared import IntrospectionCall, IntrospectionResult, MinionInvocationPack
...
class MinionWebBrokerTests(unittest.TestCase):
    ...
    def test_sandbox_client_requires_assignment_token_and_uses_unix_only(self) -> None:
        broker = MinionBrokerWebClient(Path("/tmp/pal-web-broker"), "run-web")
        with patch.dict(
            os.environ,
            {
                "PAL_MINION_WEB_BROKER": "1",
    ...
    def test_manager_validates_assignment_scope_and_web_input_before_host_call(self) -> None:
        async def scenario() -> None:
            with tempfile.TemporaryDirectory(prefix="pal_minion_web_broker_") as tmp:
                manager = MinionManager(Path(tmp))
                pack = MinionInvocationPack(
                    invocation_id="web-invocation",
                    allowed_capabilities=["op_web_search"],
                )
                manager.runs["run-web"] = MinionRunState(
                    minion_id="session-web",
                    run_id="run-web",
                    pack=pack,
                )
                ...
                manager.runs["run-web"].pack = MinionInvocationPack(
    ...
    def test_role_gateway_authentication_wraps_web_broker_methods(self) -> None:
        async def scenario() -> None:
            with tempfile.TemporaryDirectory(prefix="pal_minion_web_auth_") as tmp:
                manager = MinionManager(Path(tmp))
                manager.runs["run-web"] = MinionRunState(
                    minion_id="session-web",
                    run_id="run-web",
