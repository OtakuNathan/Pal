    ...
    async def test_real_file_calls_diff_before_paging_and_indirect_dedup(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                events.clear()
                result = await call('edit_file',{'file_path':path,'old_string':'before','new_string':'after'})
                self.assertTrue(result.ok, result.text)
                ...
                events.clear()
                result = await call('edit_file',{'file_path':path,'old_string':'missing','new_string':'bad'})
