...
class FrameLoopBoundaryTests(unittest.IsolatedAsyncioTestCase):
    async def test_frame_loop_delivers_ordinary_rejects_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            try:
                ...
                try:
                    frames = [
                        "hello world",                                   # ordinary -> delivered
    ...
    async def test_frame_loop_decodes_binary_frames_as_text(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            runtime_root = Path(raw_root)
            socket_path = runtime_root / "pal.sock"
            ...
            try:
                ...
                token = sidecar._current_peer.set("peer-1")
                try:
                    # A bytes (binary) frame decodes to its text and is delivered.
            ...
            self.assertEqual(user_messages[0].get("text"), "binary hello")
    ...
    async def test_delivery_failure_to_socket_channel_is_reported_not_raised(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            runtime = sidecar._SidecarRuntime(
                config=SidecarConfig(
                    runtime_root=runtime_root,
                    socket_channel_path=runtime_root / "missing.sock",
                )
            )
            sidecar._RUNTIME = runtime
            token = sidecar._current_peer.set("peer-1")
            try:
                fake_ws = _FakeWebSocket(frames=["hello"])
...
class BackoffAndReplyEdgeTests(unittest.IsolatedAsyncioTestCase):
    async def test_backoff_wait_interrupted_by_shutdown(self) -> None:
        runtime = sidecar._SidecarRuntime(
            config=SidecarConfig(
                ...
                socket_channel_path=Path("/tmp/pal.sock"),
                reconnect_initial_delay_seconds=30.0,
                reconnect_max_delay_seconds=30.0,
            )
        )
    ...
    async def test_forward_reply_drops_silently_when_peer_gone(self) -> None:
        runtime = sidecar._SidecarRuntime(
            config=SidecarConfig(Path("/tmp"), Path("/tmp/pal.sock"))
        )
        # Delivery registered for a peer that has since disconnected.
        runtime.deliveries["req-gone"] = "peer-gone"
        sidecar._RUNTIME = runtime
        try:
            # No peer object present -> reply dropped, never raised.
            await sidecar.forward_reply(
                {"type": "text_delta", "request_id": "req-gone", "text": "late"},
                config=runtime.config,
            )
        finally:
            sidecar._RUNTIME = None

    async def test_forward_reply_routes_to_correct_concurrent_peer(self) -> None:
        # Concurrent deliveries must route replies back to the originating peer
        # only, never crossing (request_id keyed routing).
        runtime = sidecar._SidecarRuntime(
            config=SidecarConfig(Path("/tmp"), Path("/tmp/pal.sock"))
        )
        peer_a = _FakeWebSocket()
        peer_b = _FakeWebSocket()
        runtime.peers["a"] = sidecar._PeerConnection(peer_a, "a")
        runtime.peers["b"] = sidecar._PeerConnection(peer_b, "b")
        runtime.deliveries["req-a"] = "a"
        runtime.deliveries["req-b"] = "b"
        sidecar._RUNTIME = runtime
        try:
            await sidecar.forward_reply(
                {"type": "done", "request_id": "req-b", "finish_reason": "stop"},
                config=runtime.config,
            )
        finally:
            sidecar._RUNTIME = None
        self.assertEqual(peer_a.sent, [])
        self.assertEqual(len(peer_b.sent), 1)
        self.assertEqual(json.loads(peer_b.sent[0])["request_id"], "req-b")
