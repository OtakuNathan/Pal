...
from pal.bunshin.capabilities import BunshinManagerProvider
from pal.bunshin.catalog import BunshinCatalogService
from pal.bunshin.catalog_store import family_override_path, profile_override_path
...
class BunshinCatalogTests(unittest.TestCase):
    def setUp(self) -> None:
        self.root = Path(tempfile.mkdtemp(prefix="pal_bunshin_catalog_"))

    def tearDown(self) -> None:
        shutil.rmtree(self.root, ignore_errors=True)

    def test_builtin_catalog_loads_from_package_without_runtime_seeds(self) -> None:
        snapshot = BunshinCatalogService(self.root).bootstrap()

        self.assertFalse((self.root / "plugins" / "bunshin" / "profiles").exists())
        self.assertFalse((self.root / "plugins" / "bunshin" / "families").exists())
    ...
    def test_sidecar_bootstrap_ignores_legacy_plugin_seed_catalog(self) -> None:
        legacy_root = self.root / "plugins" / "bunshin" / "profiles"
        ...
        self.assertNotEqual(BunshinProfileRegistry(runtime_root=self.root).get("generic").display_name, "Old Seed")
        self.assertIsNone(BunshinProfileRegistry(runtime_root=self.root).get("custom_writer"))
    ...
    def test_profile_override_is_atomic_versioned_and_resettable(self) -> None:
        service = BunshinCatalogService(self.root)
        ...
        effective = BunshinProfileRegistry(runtime_root=self.root).get("software_engineering.v2_coder")
        ...
        self.assertEqual(
            BunshinProfileRegistry(runtime_root=self.root).get("software_engineering.v2_coder").display_name,
            "V2 Contract Coder",
        )
    ...
    def test_family_override_validates_roles_and_restores_builtin(self) -> None:
        ...
        self.assertEqual(
            BunshinFamilyRegistry(runtime_root=self.root).get("software_engineering").display_name,
    ...
    def test_family_override_tool_uses_only_the_executor_binding_contract(self) -> None:
        parsed = (
            BunshinV2CapabilitiesBunshinV2PublicProviderSetFamilyOverrideInput.model_validate(
                {
                    "family": "lifestyle",
                    "changes": {
                        "role_bindings": {
    ...
    def test_manager_owns_catalog_rpc(self) -> None:
        manager = BunshinManager(self.root)
    ...
    def test_plugin_attach_starts_sidecar_without_importing_legacy_catalog(self) -> None:
        legacy = self.root / "plugins" / "bunshin" / "profiles" / "generic.toml"
        ...
        generic = next(item for item in snapshot["profiles"] if item["name"] == "generic")
        self.assertEqual(generic["source"], "builtin")
        self.assertEqual(generic["display_name"], "Generic Bunshin")
        self.assertIn("v2_candidate_builder", generic["capability_groups"])
    ...
    def test_public_catalog_capabilities_are_only_sidecar_proxies(self) -> None:
        ...
        read = provider.read_catalog(
            IntrospectionCall(name="intro_bunshin_catalog_read", meta=meta, args={"kind": "profiles"})
        )
        ...
        refreshed = provider.refresh_catalog(
            CapabilityCall(name="op_bunshin_catalog_refresh", meta=meta, args={})
        )
