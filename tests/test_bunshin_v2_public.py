...
class BunshinV2WorkerIdentityTests(unittest.TestCase):
    ...
    def test_workspace_holder_check_ignores_only_the_manager_snapshot_lock(self) -> None:
        ...
        lock_path = workspace / ".git" / "pal-bunshin-v2.snapshot.lock"
        ...
        manager_lock = WorkspaceProcessHolder(
            ...
            command="python",
            write_paths=(".git/pal-bunshin-v2.snapshot.lock",),
        )
        ...
        unexpected_holders = {
            "same process also holds a source file": WorkspaceProcessHolder(
                ...
                command="python",
                write_paths=(
                    ".git/pal-bunshin-v2.snapshot.lock",
                    "src/module.cpp",
                ),
            ),
...
class BunshinV2PublicSurfaceTests(unittest.TestCase):
    def setUp(self) -> None:
        self.runtime_root = Path(tempfile.mkdtemp(prefix="pal_bunshin_v2_public_"))
    ...
    def test_profile_override_translates_public_endpoint_name_at_manager_boundary(self) -> None:
        ...
        result = provider.set_profile_override(
            CapabilityCall(
                name="op_bunshin_catalog_set_profile_override",
    ...
    def test_preflight_finding_preserves_exact_authoring_locations(self) -> None:
        ...
        self.assertEqual(
            scope["allowed_paths"],
            [".pal-bunshin-architect/architect.yaml"],
        )
    ...
    def test_revision_scope_read_is_not_exposed_to_initial_architect(self) -> None:
        pack = BunshinInvocationPack(
            ...
            allowed_capabilities=[
                *ARCHITECT_BUILDER_CAPABILITIES,
                "op_bunshin_contract_revision_read",
            ],
        )
        ...
        self.assertNotIn("op_bunshin_contract_revision_read", initial.allowed_capabilities)
        self.assertNotIn("op_bunshin_contract_read", initial.allowed_capabilities)
            ...
            [
                {
                    "workflow_id": workflow.workflow_id,
                    "status": "completed",
                    "summary": "Bunshin workflow completed.",
                    "terminal_at": workflow.updated_at,
                    "resolved_interactions": [
                        {
                            "interaction_id": "bunshin_v2_architecture_arch-terminal-delivery",
    ...
    def test_runner_checkpoint_uses_manager_selected_paths_and_ignores_stale_files(self) -> None:
        async def scenario() -> None:
            ...
            first = BunshinRunner(
                runtime_root=self.runtime_root,
                pack=first_pack,
                bunshin_id=first_pack.invocation_id,
            ...
            second_pack = BunshinInvocationPack(
                invocation_id="inv-session-checkpoint",
                ...
                metadata={
                    ...
                    "bunshin_v2": {"role": "implementation", "mode": "repair"},
                    "agent_session": {
                        **base_session,
            ...
            second = BunshinRunner(
                runtime_root=self.runtime_root,
                pack=second_pack,
                bunshin_id=second_pack.invocation_id,
                run_id="run-session-checkpoint",
                write_event=lambda _event: None,
                read_decision=lambda: None,
            )
    ...
    def test_runner_restart_control_is_distinct_from_workflow_cancel(self) -> None:
        ...
        runner = BunshinRunner(
            runtime_root=self.runtime_root,
            pack=BunshinInvocationPack(invocation_id="inv-restart-control"),
            bunshin_id="inv-restart-control",
    ...
    def test_runner_keeps_same_invocation_alive_until_primary_submit_succeeds(self) -> None:
        ...
        runner = _CompletionGateRunner(
            runtime_root=self.runtime_root,
            pack=pack,
            bunshin_id="inv-completion-gate",
    ...
    def test_manager_bound_runner_requires_receipt_not_primary_file(self) -> None:
        pack = BunshinInvocationPack(
            ...
            workspace={"output_policy": {"primary_artifact": "coder_report.json"}},
            metadata={"bunshin_v2": {"submission_receipt_required": True}},
        )
        runner = BunshinRunner(
            runtime_root=self.runtime_root,
            pack=pack,
    ...
    def test_snapshot_rejection_becomes_the_latest_architect_instruction(self) -> None:
        ...
        rendered_input = render_bunshin_task_prompt(
            BunshinInvocationPack(
    ...
    def test_profile_worker_preserves_scheduler_lease_owner_id(self) -> None:
        ...
        def capture_sandbox_pack(_runtime_root, pack, **_kwargs):
            captured["control_route"] = dict(
                dict(pack.metadata.get("bunshin_v2") or {}).get("control_route") or {}
            )
    ...
    def test_manager_lifecycle_is_eager_on_attach_and_never_lazy_on_status(self) -> None:
        ...
        before = inspect_bunshin(provider)
    ...
    def test_manager_attach_retires_compatible_unowned_sidecar(self) -> None:
        ...
        with (
            ...
            patch("pal.bunshin.capabilities.subprocess.Popen", return_value=fake_process),
        ):
    ...
    def test_manager_rejects_pre_raii_sidecar_health(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "protocol is incompatible"):
            BunshinManagerProvider._validate_health(
                {"ok": True, "health_source": "bunshin_v2_manager", "manager_pid": 42}
            )
    ...
    def test_bunshin_plugin_exposes_only_semantic_v2_business_capabilities(self) -> None:
        ...
        try:
            bunshin_handle = core.context.module_registry.require("bunshin")
            self.assertIn(
                EventKind.BUNSHIN_CLARIFICATION_RESOLVED,
                bunshin_handle.event_handlers,
            )
            self.assertIn(
            ...
            canonical = {
                ...
                if descriptor.module_id == "bunshin"
            }
            self.assertEqual(
                canonical,
                {
                    "op_bunshin_start_workflow",
                    "op_bunshin_submit_artifact",
                    "intro_bunshin_task_search",
                    "intro_bunshin_task_status",
                    "op_bunshin_resume_workflow",
                    "op_bunshin_restart_execution",
                    "op_bunshin_resolve_triage",
                    "op_bunshin_submit_human_decision",
                    "op_bunshin_rebind_task_delivery",
                    "op_bunshin_answer_question",
                    "op_bunshin_control_workflow",
                    "op_bunshin_archive_workflow",
                    "intro_bunshin_catalog_read",
                    "op_bunshin_catalog_set_profile_override",
                    "op_bunshin_catalog_reset_profile_override",
                    "op_bunshin_catalog_set_family_override",
                    "op_bunshin_catalog_reset_family_override",
                    "op_bunshin_catalog_refresh",
                },
            )
            schemas = json.dumps(
                {
            ...
            workspace_schema = start_schema["$defs"]["BunshinV2StartWorkflowWorkspace"]
            ...
            decision_schema = next(
                ...
                for descriptor in core.context.capability_registry.descriptors.values()
                if descriptor.canonical_path == "op_bunshin_submit_human_decision"
            )
            status_schema = next(
                descriptor.InputModel.model_json_schema(mode="validation")
                for descriptor in core.context.capability_registry.descriptors.values()
                if descriptor.canonical_path == "intro_bunshin_task_status"
            )
            ...
            self.assertNotIn("op_bunshin_dispatch_workflow", canonical)
            self.assertNotIn("op_bunshin_tick_parent_dag", canonical)
            self.assertNotIn("op_bunshin_recover_work_order", canonical)
        finally:
    ...
    def test_start_pins_approved_skill_refs_for_manager_role_sessions(self) -> None:
        ...
        started = provider.start_workflow(
            CapabilityCall(
                name="op_bunshin_start_workflow",
    ...
    def test_public_provider_binds_current_workflow_without_exposing_manager_identity(self) -> None:
        ...
        artifact = restarted.submit_artifact(
            CapabilityCall(
                name="op_bunshin_submit_artifact",
                meta=meta,
                args={
    ...
    def test_task_status_projects_every_current_module_and_role_semantically(self) -> None:
        ...
        result = provider.workflow_status(
            CapabilityCall(
                name="intro_bunshin_task_status",
                meta=meta,
    ...
    def test_task_ledger_uses_fts_and_only_explicit_operation_rebinds_delivery(self) -> None:
        ...
        search = provider.search_tasks(
            IntrospectionCall(
                name="intro_bunshin_task_search",
                meta=new_meta,
                args={"query": "鸿蒙 字体"},
            )
        )
        ...
        rebound_result = provider.rebind_task_delivery(
            CapabilityCall(
                name="op_bunshin_rebind_task_delivery",
                meta=new_meta,
    ...
    def test_dead_task_channel_waits_durably_then_falls_back_to_live_recovery_socket(self) -> None:
        ...
        provider = BunshinManagerProvider(self.runtime_root, context=context)
        event = {
    ...
    def test_public_human_decision_resolves_pending_card_without_token_argument(self) -> None:
        ...
        validated = BunshinV2CapabilitiesBunshinV2PublicProviderSubmitHumanDecisionInput.model_validate(
    ...
    def test_public_triage_resolution_uses_semantic_subject_and_wakes_manager(self) -> None:
        ...
        result = provider.resolve_triage(
            CapabilityCall(
                name="op_bunshin_resolve_triage",
                meta={"actor_id": "nathan", "channel_id": "socket:test"},
                args={
        ...
        with sqlite3.connect(str(repository.db_path)) as connection:
            pending_effect_types = {
                ...
                for row in connection.execute(
                    "SELECT effect_type FROM bunshin_v2_outbox WHERE workflow_id = ? AND status = 'pending'",
                    ("wf_triage_hierarchy",),
                )
            }
    ...
    def test_manager_has_no_v1_spawn_rpc(self) -> None:
        manager = BunshinManager(self.runtime_root)
        with self.assertRaisesRegex(ValueError, "unknown Bunshin V2 manager method: spawn"):
    ...
    def test_worker_token_cannot_borrow_another_broker_run(self) -> None:
        ...
        manager.runs["run-other"] = BunshinRunState(
            bunshin_id="inv-other",
            run_id="run-other",
            pack=BunshinInvocationPack(invocation_id="inv-other"),
        )
    ...
    def test_manager_answers_one_pending_architect_question_in_place(self) -> None:
        ...
        state = BunshinRunState(
            bunshin_id="inv-architect-question",
    ...
    def test_manager_task_status_projects_pending_architect_question(self) -> None:
        ...
        manager.runs["run-architect-question"] = BunshinRunState(
            bunshin_id="inv-architect-question",
            run_id="run-architect-question",
            pack=BunshinInvocationPack(
                invocation_id="inv-architect-question",
                metadata={"bunshin_v2": {"workflow_id": "wf-architect-question"}},
            ),
            pending_clarification={
                "clarification_id": "clarification-1",
    ...
    def test_manager_routes_worker_question_through_task_delivery_outbox(self) -> None:
        manager = BunshinManager(self.runtime_root)
        state = BunshinRunState(
            bunshin_id="inv-routed-question",
    ...
    def test_graceful_manager_shutdown_requests_worker_restart_safe_point(self) -> None:
        async def scenario() -> None:
            ...
            pack = BunshinInvocationPack(invocation_id="inv-drain-worker")
            state = BunshinRunState(
                bunshin_id="inv-drain-worker",
        ...
        async def scenario() -> list[dict]:
            ...
            runner = _SingleInvocationRunner(
                ...
                pack=BunshinInvocationPack(
                    ...
                    metadata={"bunshin_v2": {"workflow_id": "wf"}},
                ),
                bunshin_id="inv_test",
                run_id="run_test",
