...
class PalControlFlowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_set_log_updates_future_turn_snapshot_only(self) -> None:
        ...
        self.core.context.port_registry["bunshin:bunshin"] = dependent
    ...
    async def test_refresh_llm_endpoint_also_refreshes_loaded_bunshin_runtime(self) -> None:
        class BunshinPort:
            calls = 0
        ...
        bunshin = BunshinPort()
        self.core.context.port_registry["bunshin:bunshin"] = bunshin
        ...
        self.assertEqual(bunshin.calls, 1)
        self.assertIn(
            "Dependent LLM runtimes refreshed: bunshin:bunshin",
    ...
    async def test_button_backed_set_log_resolves_interaction(self) -> None:
        ...
        self.assertEqual(statuses[-2].payload["spec"].text, "Debug logging enabled for new turns and Bunshin role runs.")
...
class TelegramControlBoundaryTests(unittest.IsolatedAsyncioTestCase):
    async def test_remote_approval_card_callback_completes_shared_approval(self):
        ...
        from pal.execution.native_shell.approval import ShellApprovals
    ...
    async def test_callback_query_is_normalized_to_interaction_result(self) -> None:
        ...
        self.assertEqual(payload.trusted_actor, "42")

    async def test_approval_callback_requires_paired_owner_and_ignores_forged_actor(self):
        from types import SimpleNamespace
        from unittest.mock import AsyncMock
        self.endpoint._interactive_messages['sudo-card'] = {
            'chat_id':100, 'owner_user_id':'42', 'interaction_kind':'approval_request',
            'actions':{'b0':{'action_key':'control.action.dispatch', 'action_args':{
                'action_kind':'shell_privilege_decision','target_scope':'execution',
                'target_id':'approval','args':{'decision':'accept','trusted_actor':'forged'}}}},
        }
        callback = SimpleNamespace(data='ix:sudo-card:b0',
            message=SimpleNamespace(chat=SimpleNamespace(id=100)),
            from_user=SimpleNamespace(id=43), answer=AsyncMock())
        update = SimpleNamespace(callback_query=callback)
        self.assertIsNone(await self.endpoint._interaction_result_from_update(update))
        callback.from_user.id=42
        result=await self.endpoint._interaction_result_from_update(update)
        self.assertEqual(result.trusted_actor,'42')
        callback.message.chat.id=101
        self.assertIsNone(await self.endpoint._interaction_result_from_update(update))
