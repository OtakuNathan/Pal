...
from pal.memory import L2Entry, MemoryPack, MemoryPackRequest, MemoryService, register_with_core as register_memory_with_core
...
class BehaviorSubsystemTests(unittest.TestCase):
    ...
    def test_read_tool_contracts_separate_memory_from_behavior(self) -> None:
        ...
        self.assertIn("Not for behavior routing rules", recall_description)
    ...
    def test_behavior_prompt_leaves_static_routing_to_tool_descriptions(self) -> None:
        ...
        self.assertIn("Consider relevant resident and learned routes", content)
        self.assertIn("future routing advice", content)
        self.assertIn("temporary advice retires automatically", content)
        self.assertIn("Use behavior tools", content)
        self.assertIn("plugin-owned guidance is read-only", content)
    ...
    def test_behavior_guidance_renders_from_behavior_not_working_memory(self) -> None:
        ...
        self.assertNotIn("Active Behavior Guidance", behavior_by_title)
    ...
    def test_memory_prompt_leaves_static_routing_to_tool_descriptions(self) -> None:
        ...
        self.assertIn("past repair decisions", policy)
        self.assertIn("recurring failures", policy)
        self.assertIn("information missing from the current context", policy)
        self.assertIn("kind=case", policy)
        self.assertIn("Memory tools define record maintenance", policy)
